...
async function runClaudeCode(cmdText: string, a: Agent): Promise<RunResult> {
  ...
  const buildArgs = (useSid: string | null) => {
    const guard =
      ...
      '[위임 판단·제안 게이트] 작업이 크거나 병렬화가 유리하거나 이종 AI(파견 분대장 Codex/Antigravity/Grok CLI)나 ' +
      '3계층 작업팀 편성이 도움이 될 것 같다고 판단되면, 스스로 위임을 실행하지 말고 먼저 PO에게 제안하라. ' +
      '제안은 결과 보고 안에 "[위임 제안]"으로 시작하는 한 문단으로 담되 ① 누구에게(어느 파견 분대장, 또는 3계층 작업팀) ' +
      '② 무엇을 ③ 왜 위임하면 좋은지 ④ 대략 규모를 밝히고 거기서 멈춰 PO의 승인 명령을 기다려라. ' +
      'PO가 명시적으로 승인하기 전에는 파견 분대장 호출(codex/agy/grok 실행)·팀 편성·대량 서브에이전트 발동을 절대 하지 마라. ' +
      '승인 없이 처리 가능한 규모면 네가 직접 하고, 큰 규모면 제안만 하고 멈춘다.';
    const args = [
