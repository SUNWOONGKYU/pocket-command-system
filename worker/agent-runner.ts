...
//   · 감사관(이름에 '감사관' 포함) → Opus 4.8
//   · 일반 워커 → Sonnet 5 (기존 기본, 2026-07-05 지시)
// env CLAUDE_MODEL 로 둘 다 강제 재정의 가능. claude_code(구독 CLI)·claude_api(직접 호출) 두 어댑터가 공유.
const IS_AUDITOR = NAME.includes('감사관');
const WORKER_MODEL = process.env.CLAUDE_MODEL || (IS_AUDITOR ? 'claude-opus-4-8' : 'claude-sonnet-5');
...
async function runClaudeCode(cmdText: string, a: Agent): Promise<RunResult> {
  ...
  const buildArgs = (useSid: string | null) => {
    ...
    const args = [
      ...
      // 모델 명시 고정(WORKER_MODEL) — 감사관 Opus 4.8 / 워커 Sonnet 5. 미지정 시 CLI 기본이 쓰이던 것을 고정.
      '--model', WORKER_MODEL,
