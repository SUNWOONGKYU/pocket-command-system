...
// 실행 모델 — PO 지시(2026-07-12): 감사관은 워커보다 상위 등급이어야 감사가 제대로 된다.
//   · 전 워커·감사관 동일 → Sonnet 5 (PO 지시 2026-07-15: 모델별 구분 없음. 모델별 주간 캡을
...
//   ★ 한시적(PO 지시 2026-07-25): 전 워커·감사관 Opus 5 로 운용. 해제 시 이 기본값만 Sonnet 5로 되돌리면 됨.
const WORKER_MODEL = process.env.CLAUDE_MODEL || 'claude-opus-5';
...
//   legacy PCS_WORKTREE alias는 2026-07-25 PO 지시로 일괄 폐지 — PCSS_WORKTREE만 유효.
const PCSS_WORKTREE = process.env.PCSS_WORKTREE === '1';
// 기존 pcs/<워커> 브랜치의 잔존 worktree는 mergeBack이 실제 HEAD 브랜치를 읽어 처리하므로 안전.
const WORKTREE_BRANCH_PREFIX = 'pcss';
...
// ── 워커 worktree 격리 헬퍼 (PCSS_WORKTREE=1 일 때만 호출됨) ─────────
...
//            사람 작업 중은 정상 상황이지 경보 대상이 아니다 — 워커 커밋은 pcss/<워커>(legacy pcs/<워커>) 브랜치에 그대로
...
async function mergeBack(repoRoot: string, worktree: string, worker: string, sourceChatId: number | null): Promise<void> {
  try {
    const base = gitSync('rev-parse --abbrev-ref HEAD', repoRoot);
    let branch = `${WORKTREE_BRANCH_PREFIX}/${worker.replace(/\s+/g, '_')}`;
    try {
      const actualBranch = gitSync('rev-parse --abbrev-ref HEAD', worktree);
      if (actualBranch && actualBranch !== 'HEAD') branch = actualBranch;
    } catch { /* branch lookup failure falls back to expected prefix */ }
...
async function runClaudeCode(cmdText: string, a: Agent): Promise<RunResult> {
  ...
  // ── worktree 격리(PCSS_WORKTREE=1) — 옵트인, 실패 시 항상 기존 a.workdir로 폴백 ──────
  //   플래그가 꺼져 있으면 이 블록은 아무 것도 하지 않는다(execCwd = a.workdir, 기존과 동일).
  let execCwd = a.workdir || undefined;
  let worktreePath: string | null = null;
  if (PCSS_WORKTREE && a.kind === 'claude_code' && a.workdir && fs.existsSync(path.join(a.workdir, '.git'))) {
  ...
  const buildArgs = (useSid: string | null) => {
    const guard =
      ...
      '[파견 분대장 vs 용병 분기 — 결정론적] 이종 AI(Codex/Antigravity/Grok)를 쓸 때는 작업 대상 폴더에 ' +
      '그 벤더 전용 2폴더(<vendor>-worktree + <vendor>-artifacts, 예: codex-worktree·codex-artifacts)가 ' +
      '둘 다 있는지부터 확인하라. 둘 다 있으면 그 벤더는 "파견 분대장"이다 — 그 worktree에서 실작업시키고 ' +
      '커밋 SHA + artifacts 검증보고서를 받아 네가 검토 후 통합한다. 폴더가 없으면 그 벤더는 "용병"이다 — ' +
      '파일 수정·커밋을 시키지 말고 단발 자문(의견·교차검증)만 받아라. 전용 폴더 유무가 곧 지휘관이 정한 스위치다. ' +
      '[위임 판단·제안 게이트] 작업이 크거나 병렬화가 유리하거나 파견 분대장·3계층 작업팀 편성이 도움이 될 것 같다고 ' +
      '판단되면, 스스로 위임을 실행하지 말고 먼저 PO에게 제안하라. 제안은 결과 보고 안에 "[위임 제안]"으로 시작하는 ' +
      '한 문단으로 담되 ① 누구에게 ② 무엇을 ③ 왜 ④ 대략 규모를 밝히고 거기서 멈춰 PO의 승인 명령을 기다려라. ' +
      'PO가 명시적으로 승인하기 전에는 파견 분대장 실작업 호출·팀 편성·대량 서브에이전트 발동을 절대 하지 마라 ' +
      '(용병 단발 자문은 승인 없이 가능). 승인 없이 처리 가능한 작은 규모면 네가 직접 하고, 큰 규모면 제안만 하고 멈춘다.';
    const args = [
      ...
      // 모델 명시 고정(WORKER_MODEL) — 전 워커·감사관 Sonnet 5(PO 2026-07-15). 미지정 시 CLI 기본이 쓰이던 것을 고정.
      '--model', WORKER_MODEL,
...
//   작업 커맨드는 PO 또는 승인된 PCSS 지원 절차만 적재하므로, 커맨드에 아래 토큰이 있으면 그 자체가 PO 승인이다.
...
function detectLimit(output: string): { resetAt: number; raw: string } | null {
  ...
  const t = output.trim();
  // 실제 한도 메시지는 짧고 그 문구가 출력 전체다(실측 ~55자). 감사문·코드리뷰가 이 문구를 '인용'해
  // 긴 본문 속에 섞인 경우는 오탐이므로, 출력이 짧을 때만(문구가 사실상 전부일 때) 인정한다.
  //   ★ 이 방어가 필요한 이유: agent-runner 자신의 커밋 diff에 이 예시 문구가 코드로 박혀 있어,
  //   감사관이 그 커밋을 감사하며 문구를 인용하면 오탐으로 워커가 잘못 보류될 수 있다.
  if (t.length > 240) return null;
  const m = t.match(/hit your [\w.\- ]*?limit\s*[·:\-]?\s*resets\s+([^\n(]+)/i);
  if (!m) return null;
...
(async () => {
  ...
  //    돈 근본 원인.) host 미등록(null) legacy row만 예외로 아무 PC에서나 허용.
  ...
  console.log(`[${NAME}] 워커 기동 · host=${self.host ?? '(무제한)'} · kind=${self.kind} · workdir=${self.workdir ?? '-'} · worktree=${PCSS_WORKTREE ? 'ON' : 'off'}`);
  // worktree 격리 사용 시 orphan worktree(수동 삭제된 폴더 등) 정리 — 1회, best-effort.
  if (PCSS_WORKTREE && self.workdir) {
