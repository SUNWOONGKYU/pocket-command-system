...
(async () => {
  ...
  // ★ 워커 worktree 격리(PCSS_WORKTREE) 도입 후: 커밋이 worktree cwd에서 만들어지면 process.cwd()가
  ...
  //   워커 데몬은 claude 하위 프로세스에 PCSS_ACTOR=<워커명>을 심어 실행한다(agent-runner.ts).
  ...
  const actor = process.env.PCSS_ACTOR === cfg.worker ? 'daemon' : 'interactive';
