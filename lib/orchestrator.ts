...
// ★ 띄어쓰기·대소문자 무시로 비교 — "루프 엔지니어링 집필가" 처럼 띄어써도 "루프엔지니어링 집필가"에 매칭.
// 긴 이름 우선 — 부분일치 오인 방지(예: "집필가"보다 "루프엔지니어링 집필가" 먼저).
...
const KEYWORD_RULES: { keywords: string[]; role: RegExp }[] = [
  // 주식 전용(2소대) — 이름 안 불러도 주식 명령은 주식 트레이더로 (분석·정보용)
  // ※ 실행성 단어(매매·매수·매도)는 제외 — 실제 매매는 별도 봇이 담당
