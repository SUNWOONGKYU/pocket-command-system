...
| **소대** | **워커 한 개** | 실제로 일하는 AI 인스턴스(소대장). claude CLI(구독) 기반. |
...
- **감사 ↔ 대응 루프**: 감사 의견에 워커가 자동 답변하고 기록을 남긴다. 대시보드에서는 감사관 발언이
...
### 8.2 데이터 모델 (핵심 2테이블)
- `agents`: 워커 명단. `name`·`role`·`squad`(중대)·`kind`(python/claude_code/claude_api)·
