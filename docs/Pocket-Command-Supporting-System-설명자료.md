...
> - **2026-07-27 현행화 ②**: 「★ 특별 기능 — 백호 소대 편제」 장을 삭제했다. 백호는 워커가 호출할 수 있는 **별도 스킬**(자체 특허)이지 이 시스템의 기능이 아니며, 저장소에 관련 코드가 없다. 남의 제품을 자기 기능으로 소개하던 서술과 Claude 상위 플랜 전용 조건(Opus 4.8 · Max/Team/Enterprise)을 함께 걷어냈다. Hermes 비교의 차별점도 실제 근거(벤더 교차 운용·교차 감사·감사 기록 무결성)로 교체.
...
1. **벤더 교차 운용·교차 감사** — Claude Code·Codex·Grok·Antigravity가 같은 큐에서 돌고, **한 벤더의 산출물을 다른 벤더의 에이전트가 읽기 전용으로 검수**한다. 외부 CLI 프로세스에는 자격증명을 넘기지 않고 쓰기 범위도 자기 작업폴더로 제한한다.
2. **감사관 견제 거버넌스** — 자기 학습이 아니라 **독립된 감사관**이 워커 커밋을 자동 감사하고, 판정이 `[조치필요]`일 때만 워커가 대응한다. 산출물 품질을 *별도 주체*가 견제.
3. **감사 기록 무결성** — 감사 원본을 작업자 저장소 **밖**에 두고 SHA-256 해시 체인으로 봉인한다. 해시 계산은 감사받는 AI가 아니라 별도 스크립트가 수행하므로, 검사받는 쪽이 자물쇠를 만들 수 없다.
4. **군대 편제 UX(사용자 경험) + 부대 관제 대시보드** — 지휘관·지휘소·중대·소대·분대·감사관의 일관된 지휘 메타포 + 하트비트 EKG 대시보드(Pocket Command Post). (Hermes Agent Kanban이 *작업* 중심 보드라면, 이쪽은 *부대 생존·상태* 중심 관제.)

### 한 줄 정리
> Hermes Agent는 **"스스로 자라는 범용 에이전트 런타임"**(원조·성숙·다채널·학습·이식성).
> Pocket Command Supporting System은 그걸 벤치마킹한 **"여러 벤더 AI를 폰에서 지휘하고 서로 감사시키는 거버넌스"** 버전 —
> *개인이 폰으로 다수 PC의 AI 부대를 지휘하고, 서로 다른 회사의 AI가 서로를 감사하게 하며, 그 기록을 위변조 탐지되게 봉인*하는 데 특화했다.
> **닮은 토대 위에, 벤더 교차 감사·기록 무결성·편제 UX를 얹은 것**이 본질이다.
