...
## 목차
1. [개요 — 한눈에 (입문자용)](#1-개요--한눈에)
2. [무엇이 다른가](#2-무엇이-다른가)
3. [핵심 개념 — 군대 편제로 이해하기](#3-핵심-개념--군대-편제로-이해하기)
4. [어떻게 동작하나 — 작업 한 건의 여정](#4-어떻게-동작하나)
5. [주요 기능](#5-주요-기능)
5+. [★ 특별 기능 — 백호 소대 편제 (대규모 AI 동시 투입)](#-특별-기능--백호-소대-편제-대규모-ai-동시-투입)
5++. [비교 — Hermes Agent와의 관계 (벤치마킹 출발점)](#비교--hermes-agent와-pocket-command-system)
6. [기술 구조 (실무자용)](#6-기술-구조-실무자용)
7. [직접 만들기 — 재현 가이드](#7-직접-만들기--재현-가이드)
8. [한계와 주의점 (정직하게)](#8-한계와-주의점)
9. [용어집](#9-용어집)
...
- 전개 단계:
  ...
  소대장(워커 1)
     └─ 백호 → 일반 분대 완편 (A형 44명)
            └─ + DW 특수부대 추가 → 각 DW가 동시 16 / 누적 최대 1,000 에이전트로 포격
                   = 총 동원 규모 수백~천 단위 (대규모 AI 군단)
  ```
- 즉 **PC 여러 대(중대) × 워커별 백호 완편(44명) × DW 특수부대 포격(최대 1,000)**이 곱해지면,
  폰 한 줄 명령이 수백~천 AI의 동시 작전으로 전개됩니다.
...
Pocket Command System은 잘 알려진 에이전트 하네스 **Hermes Agent**(NousResearch)를 **벤치마킹해 만든
Claude Code 네이티브 재구현**입니다. 둘은 닮은 점이 많고, 그것을 과장 없이 정직하게 정리합니다.

> ⚖️ **오해 정정**: "멀티 워커·오케스트레이션·공유 작업보드·멀티호스트·텔레그램"은 **헤르메스에 이미 다 있습니다.**
> 이것들은 우리만의 차별점이 아니라 **헤르메스에서 가져온 공통 토대**입니다. 처음 비교에서 이를 우리 강점처럼
> 적었던 것은 오류이며 바로잡습니다.

### 공통 토대 (헤르메스가 원조, 우리도 채택)
- **멀티 워커 + 오케스트레이션** — 헤르메스는 `role="orchestrator"` 자식이 워커를 스폰(트리 깊이 `max_spawn_depth`).
- **공유 작업보드** — 헤르메스 **Kanban**: 작업=DB 행, 워커=독립 OS 프로세스, 프로필 간 공유 (우리 Supabase `tasks` 큐와 동형).
- **멀티호스트** — 헤르메스 6개 백엔드(local·Docker·SSH·Daytona·Singularity·Modal).
...
1. **Claude Code 네이티브 대량 폭발** — 워커가 claude CLI(구독) 위에서 돌고, **백호(Agent Teams) 완편 44명 + DW(Dynamic Workflows) 최대 1,000**으로 단일 명령을 군단급으로 전개. 헤르메스 일반 서브에이전트와 다른 *Claude 전용* 메커니즘.
2. **감사관 견제 거버넌스** — 자기 학습이 아니라 **독립된 감사관**이 워커 커밋을 자동 감사 → 워커가 자동 대응(감사↔대응 루프). 산출물 품질을 *별도 주체*가 견제.
3. **군대 편제 UX + 부대 관제 대시보드** — 지휘관·참모장·중대·소대·분대·감사관의 일관된 지휘 메타포 + 하트비트 EKG 대시보드(Pocket Command Post). (헤르메스 Kanban이 *작업* 중심 보드라면, 이쪽은 *부대 생존·상태* 중심 관제.)

### 한 줄 정리
> 헤르메스는 **"스스로 자라는 범용 에이전트 런타임"**(원조·성숙·다채널·학습·이식성).
...
좋은 점만 말하면 거짓이라, 실제로 커질 때 물리는 지점을 적습니다.
